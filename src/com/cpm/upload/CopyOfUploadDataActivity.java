...
public class CopyOfUploadDataActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.copy_upload_layout);
	...
	public void onBackPressed() {
		...
		startActivity(i);
		CopyOfUploadDataActivity.this.finish();
	}
						...
						if (!coverageBeanlist.get(i).getStatus()
							...
							if (!(coverageBeanlist.get(i).getReasonid()
								...
								for (int j = 0; j < openingData.size(); j++) {
									...
									String finalOpeningStock = makeJson(openingData
											.toString());
...
//								test = makeJson(test);
								...
								*/}
								...
								for (int j = 0; j < promotionData.size(); j++) {
									...
									onXML = "[USER_DATA][MID]"
											...
											+ "[/SKU_CD]"
											+ "[PROMOTION]"
											+ promotionData.get(j).getPromotion_name()
											+ "[/PROMOTION][PRESENT]"
											+ promotion_avail
											+ "[/PRESENT] [REMARK]"
								...
								promotion_xml = "\""+"[DATA]" + promotion_xml +"[/DATA]"+"\"";
									...
									promoData.put("KEYS","PROMOTION_DATA_NEW");
								...
								if (lunchData.size()>0) {
									...
									String finalSalesPer1 = makeJson(obj
											.toString());
								...
								if (new File(Environment.getExternalStorageDirectory()+"/Pillsbury_Images/"
									...
									result = UploadPOSMImage(
											...
											coverageBeanlist.get(i).getImage(), "DealerboardImage");
								...
								for(int j=0; j< AssetData.size();j++){
									if (AssetData.get(j).getImage() != null
											&& !AssetData.get(j).getImage()
													.equals("")) {
									...
									if (new File(Environment.getExternalStorageDirectory()+"/Pillsbury_Images/"
										...
										result = UploadPOSMImage(
												...
												AssetData.get(j).getImage(), "AssetImage");
								...
								promotionData = database.getPromotionDataUpload(coverageBeanlist
										.get(i).getStoreId());
								
								
								
								for(int j=0;j<promotionData.size();j++){
									if (promotionData.get(j).getImage() != null
											&& !promotionData.get(j).getImage()
													.equals("")) {
									...
									if (new File(Environment.getExternalStorageDirectory()+"/Pillsbury_Images/"
										...
										result = UploadPOSMImage(
												...
												promotionData.get(j).getImage(), "PromotionImage");
								...
								posmData = database.getProductEntryDetail(coverageBeanlist
										.get(i).getStoreId());
								
								
								
								for(int j=0;j<posmData.size();j++){
									if (posmData.get(j).getImage() != null
											&& !posmData.get(j).getImage()
													.equals("")) {
									...
									if (new File(Environment.getExternalStorageDirectory()+"/Pillsbury_Images/"
										...
										result = UploadPOSMImage(
												...
												posmData.get(j).getImage(), "AdditionalVisibility");
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals(CommonString.KEY_SUCCESS)) {
				...
				AlertMessage message = new AlertMessage(
						CopyOfUploadDataActivity.this, CommonString.ERROR + result, "success", null);
				message.showMessage();
			}
	...
	public String UploadPOSMImage(String path, String folder) throws Exception {
		...
		Bitmap bitmap = BitmapFactory.decodeFile(
				Environment.getExternalStorageDirectory()+"/Pillsbury_Images/" + path, o2);
