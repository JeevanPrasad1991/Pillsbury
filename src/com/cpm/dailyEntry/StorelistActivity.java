...
public class StorelistActivity extends ListActivity {
	...
	private static ArrayList<PromotionTrackingBean> promotion_list = new ArrayList<PromotionTrackingBean>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String visit_storeid = preferences.getString(
		...
		if (visit_storeid != null && !visit_storeid.equals("")) {
			...
			if (storestatus.getCheckout_status().equalsIgnoreCase(
					...
					|| storestatus.getCheckout_status().equalsIgnoreCase(
							CommonString.KEY_C)) {

			} else {

				database.updateStoreStatusOnCheckout(visit_storeid, date,
						CommonString.KEY_INVALID);
			}
		}
	...
	private void filldata() {
		...
		for (int i = 0; i < coverageBeanlist.size(); i++) {
			...
			boolean opening = false, closing = false, stock = false, promotion = false, asset = false, call= false, competition_sos = false;
			...
			if ((storestatus.getCheckout_status().equalsIgnoreCase(
				...
				promotion_list = database.getPromotionTrackData(store_id);
				...
				competitionlist = database.getInsertedCompetitionData(store_id);
				...
				if(!RightsTypes.equalsIgnoreCase("Merchandiser"))
				{
					...
					if (opening && closing && call && stock && promotion && asset && competition_sos) {
						database.updateStoreStatusOnCheckout(coverageBeanlist
								.get(i).getStoreId(), date, CommonString.KEY_VALID);
					}
	...
	private class MyAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			} else if ((list.get(position).getCheckout_status()
		...
		if ((sb.getStatus().equals(CommonString.KEY_U))) {
			...
			if (preferences.getString(CommonString.KEY_STOREVISITED_STATUS,
				...
				if (preferences.getString(CommonString.KEY_STOREVISITED, "")
					...
					Toast.makeText(
							StorelistActivity.this,
							"You Can't Checkin,First Checkout the Previous Partner",
				...
				editor.putString(CommonString.KEY_STORE_CATEGORY,
						sb.getCategory());
