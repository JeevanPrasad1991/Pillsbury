...
public class XMLHandlers {
	...
	public static MappingPromotionpostgetterSetter NonPromotionalXMLHandlerData(XmlPullParser xpp, int eventType) {
		MappingPromotionpostgetterSetter jcpGetterSetter = new MappingPromotionpostgetterSetter();

		try {
			while (xpp.getEventType() != XmlPullParser.END_DOCUMENT) {
				if (xpp.getEventType() == XmlPullParser.START_TAG) {
					if (xpp.getName().equals("ID")) {
						jcpGetterSetter.setID(xpp.nextText());
					}
					if (xpp.getName().equals("STORE_CD")) {
						jcpGetterSetter.setSTORE_CD(xpp.nextText());
					}
					if (xpp.getName().equals("SKU_CD")) {
						jcpGetterSetter.setSKU_CD(xpp.nextText());
					}
					if (xpp.getName().equals("PROMOTION")) {
						jcpGetterSetter.setPROMOTION(xpp.nextText());
					}
