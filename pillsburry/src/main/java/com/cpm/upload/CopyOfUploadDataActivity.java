...
//No data
                                ...
                                if (listedStockData.size() > 0) {
                                    ...
                                    for (int j = 0; j < listedStockData.size(); j++) {
                                        onXML = "[USER_DATA]"
                                                ...
                                                + "[/BRAND_CD]"
                                                + "[/USER_DATA]";
                                        stock_xml1 = stock_xml1 + onXML;

                                    }
                                    stock_xml1 = "\"" + "[DATA]" + stock_xml1 + "[/DATA]" + "\"";
                                    //stock_xml1 = "[DATA]" + stock_xml1 + "[/DATA]";
                                    JSONObject stockData = new JSONObject();
                                    stockData.put("MID", mid);
                                    stockData.put("KEYS", "LISTED_SKU_DATA");
                                    stockData.put("XMLDATA", stock_xml1);
                                    stockData.put("USERNAME", username);
                                ...
                                if (new File(Environment.getExternalStorageDirectory() + "/Pillsbury_Images/" + coverageBeanlist.get(i).getImage()).exists()) {
                ...
                final AlertMessage message = new AlertMessage(CopyOfUploadDataActivity.this, AlertMessage.MESSAGE_EXCEPTION, "download", e);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        message.showMessage();
                ...
                e.printStackTrace();
                final AlertMessage message = new AlertMessage(CopyOfUploadDataActivity.this,
        ...
        protected void onPostExecute(String result) {
            ...
            dialog.dismiss();
            if (result.equalsIgnoreCase(CommonString.KEY_SUCCESS)) {
                AlertMessage message = new AlertMessage(CopyOfUploadDataActivity.this, AlertMessage.MESSAGE_UPLOAD_DATA, "success", null);
                ...
                Toast.makeText(getApplicationContext(), "error " + CommonString.ERROR, Toast.LENGTH_SHORT).show();
