...
//			compliance_list = database.getServiceData(storetype_id);
            for (int i2 = 0; i2 < compliance_list.size(); i2++) {
                ...
                sub_compliance_list = database.getSubskulistData(compliance_list.get(i2).getBRAND_CD(), store_id);
...
//					sub_compliance_list.get(i).setListedFlag("true");
            ...
            for (int i2 = 0; i2 < compliance_list.size(); i2++) {
                ...
                listDataHeader.add(chk);
                sub_compliance_list = database.getInsertedStockRecievedSubList(compliance_list.get(i2).getCommonId());
                listDataChild.put(chk, sub_compliance_list);
                listDataChild.put(chk, addData(sub_compliance_list));
...
//			midStock = database.getMidDayStockForCheck(subList.get(i).getSKU_CD());
    ...
    private class ExpandableListAdapter extends BaseExpandableListAdapter {
        ...
        public View getChildView(final int groupPosition,
            ...
            final MidDayRecievedStockBean childText = (MidDayRecievedStockBean) getChild(groupPosition, childPosition);
...
//			brand_name.setText(childText.getBRAND());
    ...
    boolean validate(HashMap<MidDayRecievedStockBean, List<MidDayRecievedStockBean>> listDataChild2,
